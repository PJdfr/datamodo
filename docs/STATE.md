...
| **Folder structure from the graph** (Build ▾ → "Export as folders"): describe an organization → LLM files the exportable nodes (documents + body nodes; it sees an INVENTORY — labels/kinds/links, never contents) into a sanitized folder plan → tree PREVIEW (unplaced items land in `unsorted/`, nothing silently dropped) → download builds a .zip: originals streamed from blob storage (fail-soft to markdown), other nodes as cited markdown pages, + README of the tree; dependency-free store-only zip writer | pure `lib/datamodo/folder-export.ts` + `lib/datamodo/zip.ts`, `app/api/knowledge/folder-export`, `app/dashboard/folder-export-modal.tsx` |
