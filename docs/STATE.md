...
| **Data tab IA** (flat + unified, 2026-07-11): ONE toggle — ▦ Tables · ◍ Explore · Timeline · Files · Insights. **Tables = the unified surface**: Supabase-style schema diagram (kind cards with columns + FK relation rows + measured relation lines), click a card → browse rows as entity cards, "▦ open/make table", and "+ new table" creates category + dataset as ONE object; the dataset list sits below | `app/dashboard/schema-view.tsx`, `knowledge-view.tsx` (`view: "schema"\|"explore"`), `control-center.tsx` (`DataView`) |
