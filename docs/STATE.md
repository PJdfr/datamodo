...
| **Explorer v2 — the 3D graph walk**: depth-field ego graph (center forward, hop-1 ring, hop-2 hazy behind cream fog), world-reflow walk with enter-from-parent/recede animations, floating breadcrumb + jump box, edge inspector (confidence meter · since · corroboration pips · quoted evidence), natural-shape side panel; flat 2D radial under `prefers-reduced-motion`. **Ring grouping (WOW build-order #1)**: each ring's capped long tail collapses per kind into an expandable "+38 more invoices" pseudo-node (dashed chip on the ring, dashed spoke to its anchor); click/Enter reveals the next 6 by rank (bloom out of the chip), expansion is session-local and resets on walk | pure `lib/datamodo/explorer.ts` (`buildEgoGraph` + `parentOf`/`groups`/`pinned`, `pluralizeKind`, `depthLayout`, `DEPTH`), `app/dashboard/explorer-view.tsx` (`Group3D`, `GROUP_PAGE`); design source: Claude Design project "Datamodo Explorer v2" |
