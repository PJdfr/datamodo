...
| Knowledge graph + curation: drag-to-pin (persisted `entities.graph_pin`), kind hypernodes — now the Explorer's ⌂ zoomed-out overview, not a separate view | `app/dashboard/knowledge-graph.tsx` (`onWalk`), `PATCH app/api/knowledge/entities/[id]` |
...
| **Data tab IA** (flat, 2026-07-11): ONE toggle — Tables · ◍ Explore · Cards · Concepts · Timeline · Files · Insights; no sub-modes. Explore zooms out (⌂) to the merged-in Map (pins + kind clusters, "◍ Walk from here" dives back); Cards groups bridge to Tables ("▦ open as table" → idempotent category→table); build actions behind one "✦ Build ▾" menu | `app/dashboard/control-center.tsx` (`DataView`), `knowledge-view.tsx` (`view` prop), `explorer-view.tsx` (`zoomedOut`), `knowledge-graph.tsx` (`onWalk`) |
