...
| **In-app Chat channel (2026-07-12)**: the app itself is a capture channel, peer to email/WhatsApp — a Chat tab with a thread + composer: multiline text, any-file attach (photos/PDF/docs/sheets, ≤8 files, 15 MB/message), dictaphone voice notes (MediaRecorder → the audio/transcription tier), live speech-to-text into the box (Web Speech API, feature-detected); messages land as `upload`-channel items (`meta.via = "app"`) with the same post-response extraction kick as /api/ingest; the thread shows each message's live pipeline state (⟳ processing → ✓ filed / ⚠, 4s poll while busy) | `app/api/chat/route.ts` (session-authed, explicit org attribution), `app/dashboard/chat-view.tsx`, Chat tab in `control-center.tsx` |
