...
subgraph PROCESS["4 · WE PROCESS<br/>LLM extraction, steered by the user's ontology"]
  ...
  TICK --> MSG & DOC & IMG & NOTE
  KINDS -.steers.-> MSG & DOC & IMG
  MSG & DOC & IMG & NOTE --> CANON
end
...
| **Process** | Facts appear minutes later; unclear things ask for review | Cron + self-kick drain; claim/recover/retry; classify → extract → canonicalize → restrain; low confidence escalates models, then to Review | `lib/datamodo/extract.ts`, `ontology.ts`, `documents.ts`, `lib/llm/*` |
...
6. **Failure degrades, never breaks** — no key → no embeddings/vision, item still lands; bad attachment → `metadata_only` node.
