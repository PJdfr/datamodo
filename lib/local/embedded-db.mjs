...
// On Windows the npm shims are `.cmd` files, which Node's spawn (no shell) can't
// resolve from the bare name "npx" — it errors ENOENT. Use the right name per OS.
...
export async function startEmbeddedDb({ appRoot, dataDir, port = 54321, host = "127.0.0.1" }) {
  ...
  async function ensureSchema(appVersion, opts = {}) {
    ...
    if (have.trim() === `${appVersion}:${embeddingDim}`) return;
    ...
    const status = await new Promise((res) => {
      ...
      p.on("exit", res);
      // Surface WHY it couldn't even start (e.g. npx/prisma not found) instead
      // of swallowing it — that silent path is what made this look opaque.
      p.on("error", (e) => { console.error(`datamodo: could not run prisma (${NPX}) — ${e?.message ?? e}`); res(1); });
    });
