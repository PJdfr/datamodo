...
export async function startConnectors({ dataDir, endpoint, secret, intervalMs = 60_000, log = console.log }, deps = {}) {
  const connectors = await loadConnectors(dataDir);
  if (!connectors.length) return { stop: noop, connectors: [] };

  const ImapFlow = deps.ImapFlow ?? (await import("imapflow")).ImapFlow;
  ...
  log(`datamodo: watching ${connectors.length} mailbox(es) over IMAP`);
  let stopped = false;
  const tick = async () => {
    for (const connector of connectors) {
      if (stopped) return;
      try {
        await pollConnector(connector, { ImapFlow, post, getCursor, setCursor, log });
      } catch (e) {
