...
the `datamodo.email` catch-all and invokes this Worker, which parses the MIME,
...
```
sender → MX (datamodo.email) → Email Routing catch-all → THIS WORKER
...
- Add `datamodo.email` as a zone in Cloudflare and point the registrar's
...
That's it — no per-user setup. Every `<token>@datamodo.email` the app mints
(`lib/datamodo/inbox.ts`) is covered by the one catch-all.

## Test it
1. Sign in to the app and copy your inbox address (`<token>@datamodo.email`).
